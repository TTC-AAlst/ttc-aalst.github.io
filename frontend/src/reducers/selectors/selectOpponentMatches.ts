...
export const selectOpponentMatches = createSelector(
  [selectReadOnlyMatches, (_, match: IMatch) => match, (_, __: IMatch, opponentIn?: ITeamOpponent) => opponentIn],
  (readOnlyMatches, match, opponentIn) => {
    const opponent = opponentIn || match.opponent;
    const matches = readOnlyMatches.filter(x => x.competition === match.competition && x.frenoyDivisionId === match.frenoyDivisionId);
