...
export const AdminPlayerLineup = () => {
  ...
  const teams = allTeams.filter(team => team.competition === state.comp).filter(team => team.players.some(ply => ply.playerId === player?.id));

  let playerFormation: React.ReactNode;
  if (state.playerId) {
    if (!teams.length) {
      playerFormation = <div>Heeft geen vaste ploeg ingesteld (Geen Standard/Captain, misschien enkel als Reserve?)</div>;
    } else {
      playerFormation = <PlayerLinup playerId={state.playerId} teams={teams} />;
    }
  }
...
const AdminPlayerLineupToolbar = ({ onFilterChange }: AdminPlayerLineupToolbarProps) => {
  ...
  return (
    <div style={{ padding: 10, display: 'inline-block', width: 300 }}>
      <PlayerAutoComplete selectPlayer={id => typeof id === 'number' && setPlayerId(id)} label="Selecteer speler" />
